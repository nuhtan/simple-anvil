...
use std::{array::TryFromSliceError, borrow::BorrowMut, cell::Cell, convert::TryInto, fs, io::{BufReader, Read}, marker::{self, PhantomData}};
...
impl<'a> Region<'a> {
    ...
    pub fn header_offset(self, chunk_x: u32, chunk_z: u32) -> u32 {
        return 4 * (chunk_x % 32 + chunk_z % 32 * 32);
    }

    pub fn chunk_location(self, chunk_x: u32, chunk_z: u32) -> (u32, u32) {
        ...
        let temp: [u8; 3] = temp_range.try_into().expect("Failed to convert slice into array.");
            
        // let temp: Result<[u8; 4], TryFromSliceError> = off_clone[b_off..b_off + 3].try_into();
